...
from campus_outline import CampusOutline, extract_campus_outer_outline
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        default=None,
        help=(
            "用于提取校园最外围红色轮廓线的对齐实测CSV或目录；"
            "默认自动读取 data/aligned_measurements。"
        ),
    )
    p.add_argument(
        "--no-campus-outline",
        action="store_true",
        help="不在联合best-server RSRP和PCI图中绘制校园外围红色轮廓线",
    )
    p.add_argument(
        "--campus-outline-linewidth",
        type=float,
        default=1.8,
        help="校园外围红色轮廓线宽度，默认1.8",
    )
    p.add_argument(
        "--campus-outline-buffer-m",
        type=float,
        default=40.0,
        help="轨迹到校园外围包络的初始缓冲距离(m)，默认40；必要时会自动增大",
    )
    p.add_argument(
        "--campus-outline-max-buffer-m",
        type=float,
        default=120.0,
        help="校园外围包络自动连接时允许的最大缓冲距离(m)，默认120",
    )
    p.add_argument(
        "--campus-outline-grid-m",
        type=float,
        default=5.0,
        help="校园曲线轮廓提取栅格分辨率(m)，默认5",
    )
    p.add_argument(
        "--campus-outline-smooth-m",
        type=float,
        default=18.0,
        help="校园曲线轮廓平滑尺度(m)，默认18；越大越圆滑",
    )
    p.add_argument(
        "--campus-outline-min-coverage",
        type=float,
        default=0.97,
        help="主校园包络需覆盖的轨迹栅格比例，默认0.97",
    )
    p.add_argument(
        ...
        default=220.0,
        help=(
            "把用户标出的右偏中部（S38附近）小弯折替换为单一直线段；"
            "数值为目标点两侧各保留的弧长(m)，默认220；设为0可关闭"
        ),
    )
    ...
    p.add_argument(
        ...
        action="store_true",
        help=(
            "不重新运行Sionna；直接读取已保存的joint_best_server_27stations_4000x3000.npz"
            "并重新生成联合地图PNG（含校园红色外围轮廓）。"
        ),
    )
...
def _resolve_paths(args: argparse.Namespace) -> dict[str, Any]:
    ...
    campus_outline_source = (
        Path(args.campus_outline_source).expanduser().resolve()
        if args.campus_outline_source
        else root / "data" / "aligned_measurements"
    )
    return {
        "root": root,
        "summary": summary,
        ...
        "campus_outline_source": campus_outline_source,
    }
...
def _assemble(
    ...
    _plot_outputs(args, output_root, arrays, sector_catalog, x_min, y_min, paths["campus_outline_source"])
...
def _save_png(fig, output_path: Path, dpi: int = MAP_DPI, facecolor: str | None = None) -> Path:
    """Save one publication PNG using a Windows-safe atomic write.

    Matplotlib/Pillow normally opens the final output path internally. On some
    Windows installations this can raise ``OSError: [Errno 22] Invalid
    argument`` even for a valid path.  To avoid that code path, the figure is
    first written to an already-open temporary binary file in the destination
    directory, and the completed PNG is then atomically moved to its final
    filename.

    Full-page canvas width is fixed to 7.48 in. No duplicate PNG is produced.
    """
    import os
    import tempfile
    ...
    if not base.parent.is_dir():
        raise NotADirectoryError(f"PNG output parent is not a directory: {base.parent}")
    ...
    # A directory accidentally created with the PNG filename cannot be
    # overwritten as a file. Report it explicitly instead of surfacing a
    # cryptic Pillow error.
    if out.exists() and out.is_dir():
        raise IsADirectoryError(
            f"PNG output path is a directory, not a file: {out}. "
            "Rename or remove that directory and rerun --plot-only."
        )

    tmp_name = None
    try:
        # Pass an already-open file object to Matplotlib.  This is important:
        # Pillow therefore never calls builtins.open() on the final Windows
        # pathname, which is where Errno 22 occurred in the reported run.
        ...
        if (not tmp_path.exists()) or tmp_path.stat().st_size <= 0:
            raise OSError(f"Temporary PNG was not written correctly: {tmp_path}")

        try:
            os.replace(str(tmp_path), str(out))
        except PermissionError as exc:
            raise PermissionError(
                f"Cannot replace PNG output: {out}. The existing image may be "
                "open in another program. Close it and rerun --plot-only."
            ) from exc
        except OSError as exc:
            # One more attempt with absolute normalized strings.  This keeps
            # the operation inside the OS rather than asking Pillow to open
            # the final path.
            final_abs = os.path.normpath(os.path.abspath(str(out)))
            tmp_abs = os.path.normpath(os.path.abspath(str(tmp_path)))
            try:
                os.replace(tmp_abs, final_abs)
            except OSError as exc2:
                raise OSError(
                    f"Failed to finalize PNG output. target={final_abs!r}; "
                    f"temporary={tmp_abs!r}. If an older PNG is open in a "
                    "viewer, close it and rerun --plot-only."
                ) from exc2

        if (not out.exists()) or out.stat().st_size <= 0:
            raise OSError(f"PNG save verification failed: {out}")
        return out
    finally:
        if tmp_name:
            try:
...
def _load_and_save_campus_outline(
    args: argparse.Namespace,
    output_root: Path,
    source: Path,
    extent,
) -> CampusOutline | None:
    """Extract and persist the campus outer envelope from aligned measurements."""
    if bool(getattr(args, "no_campus_outline", False)):
        return None
    try:
        outline = extract_campus_outer_outline(
            source,
            extent=(float(extent[0]), float(extent[1]), float(extent[2]), float(extent[3])),
            grid_m=float(args.campus_outline_grid_m),
            buffer_m=float(args.campus_outline_buffer_m),
            max_buffer_m=float(args.campus_outline_max_buffer_m),
            smooth_m=float(args.campus_outline_smooth_m),
            min_coverage=float(args.campus_outline_min_coverage),
            straight_segment_m=float(args.campus_outline_straight_segment_m),
        )
    except (FileNotFoundError, ValueError, KeyError) as exc:
        print(f"[WARN] Campus outer outline was not drawn: {exc}")
        return None

    pd.DataFrame(
        {
            "vertex_order": np.arange(len(outline.xy), dtype=int),
            "blender_x": outline.xy[:, 0],
            "blender_y": outline.xy[:, 1],
        }
    ).to_csv(
        output_root / "campus_outer_outline_from_aligned_measurements.csv",
        index=False,
        encoding="utf-8-sig",
    )
    metadata = {
        "method": outline.method,
        "coordinate_fields": ["blender_x", "blender_y"],
        "outline_grid_m": float(outline.grid_m),
        "outline_buffer_m": float(outline.buffer_m),
        "outline_smooth_m": float(outline.smooth_m),
        "trajectory_coverage_fraction": float(outline.covered_fraction),
        "source_files": list(outline.source_files),
        "distinct_source_point_count": int(outline.source_point_count),
        "outline_vertex_count": int(outline.hull_vertex_count),
        "line_color": "red",
        "line_width": float(args.campus_outline_linewidth),
        "right_middle_straight_segment_applied": bool(outline.straight_segment_applied),
        "right_middle_straight_segment_half_span_m": float(outline.straight_segment_half_span_m),
        "right_middle_straight_segment_target_xy_m": [
            float(outline.straight_segment_target_x_m),
            float(outline.straight_segment_target_y_m),
        ],
    }
    (output_root / "campus_outer_outline_metadata.json").write_text(
        json.dumps(metadata, ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
    print(
        "[OK] Campus outer outline: "
        f"{outline.source_point_count} aligned points -> "
        f"{outline.hull_vertex_count} smooth outline points; "
        f"buffer={outline.buffer_m:.1f} m, smooth={outline.smooth_m:.1f} m, "
        f"coverage={100.0 * outline.covered_fraction:.1f}%, "
        f"S38-side straight segment={'on' if outline.straight_segment_applied else 'off'}"
    )
    return outline


def _overlay_campus_outline(
    ax,
    outline: CampusOutline | None,
    *,
    linewidth: float,
) -> None:
    if outline is None or outline.xy.size == 0:
        return
    ax.plot(
        outline.xy[:, 0],
        outline.xy[:, 1],
...
def _plot_outputs(
    ...
    campus_outline = _load_and_save_campus_outline(
        args,
        output_root,
        campus_outline_source,
        extent,
    ...
    _overlay_campus_outline(
        ax,
        campus_outline,
        linewidth=args.campus_outline_linewidth,
    )
    station_points = sector_catalog.drop_duplicates("station_id")
    ax.scatter(station_points["x_m"], station_points["y_m"], marker="^", s=18,
               facecolors="none", edgecolors="red", linewidths=0.7, zorder=21)
    ax.set_title("Best-server radio map for the 27 physical base stations")
    ...
    ax.set_title("Best-server PCI in the joint radio map of the 27 physical base stations")
    ax.set_xlabel("Blender X (m)")
    ax.set_ylabel("Blender Y (m)")
...
def _plot_only_from_saved_npz(
    ...
    if not final_npz.exists():
        raise FileNotFoundError(
            f"--plot-only requires an existing joint map NPZ: {final_npz}"
        )

    print(f"[PLOT-ONLY] Loading saved joint map: {final_npz}")
    with np.load(final_npz, allow_pickle=False) as data:
        required = ("best_rsrp_dbm", "best_station_id", "best_pci", "building_mask")
        missing = [name for name in required if name not in data.files]
        if missing:
            raise KeyError(f"Saved joint map is missing arrays: {missing}")

        arrays = {name: np.asarray(data[name]) for name in required}
        if "x_centers_m" in data.files and len(data["x_centers_m"]) > 0:
            x_centers = np.asarray(data["x_centers_m"], dtype=float)
            x_min = float(x_centers[0] - 0.5 * args.cell_size)
            args.size_x = int(round((x_centers[-1] - x_centers[0]) + args.cell_size))
        else:
            x_min = float(args.center_x - args.size_x / 2.0)

        if "y_centers_m" in data.files and len(data["y_centers_m"]) > 0:
            y_centers = np.asarray(data["y_centers_m"], dtype=float)
            y_min = float(y_centers[0] - 0.5 * args.cell_size)
            args.size_y = int(round((y_centers[-1] - y_centers[0]) + args.cell_size))
        else:
            y_min = float(args.center_y - args.size_y / 2.0)
    ...
    _plot_outputs(
        ...
        x_min,
        y_min,
        paths["campus_outline_source"],
    )
    print("[PLOT-ONLY] Joint-map figures regenerated without rerunning Sionna.")
    return 0
...
def _orchestrator(args: argparse.Namespace, paths: dict[str, Any]) -> int:
    ...
    if args.plot_only:
        return _plot_only_from_saved_npz(args, paths, sector_catalog)
