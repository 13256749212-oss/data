...
# Process-local caches used by the 27-station progressive sweep.  Simulation
# maps are immutable, so discovering/loading the same PCI map hundreds of times
# is pure overhead.  At 512x512 cells the complete 79-PCI cache remains modest
# in memory while eliminating repeated recursive glob scans and NPZ decompression.
_SIMULATION_PATH_CACHE: dict[tuple[str, int, int], Path] = {}
_SIMULATION_PRIOR_CACHE: dict[tuple[str, int, int], object] = {}
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        help="纯仿真NPZ搜索根目录；默认outputs/bestparam_radio_maps_512m",
    )
    p.add_argument(
...
def random_points(points: pd.DataFrame, k: int, seed: int) -> pd.DataFrame:
    """Nested information-balanced receiver-location sampling.

    The same seed always produces one full ranking of the available measured
    receiver locations, so 10,11,...,15 point experiments remain strictly nested.
    Unlike a purely uniform permutation, each newly added location is encouraged
    to increase spatial aperture, PCI/sector completeness and RSRP diversity.
    The ranking uses measured receiver coordinates/RSRP only; transmitter truth
    and Sionna values are never used for point selection.
    """
    ...
    rss = pool[rss_cols].to_numpy(float)
    ...
    rss_span = np.zeros(len(pool), dtype=float)
    for i in range(len(pool)):
        vals = rss[i, finite[i]]
        rss_span[i] = float(np.max(vals) - np.min(vals)) if len(vals) >= 2 else 0.0
    ...
    def _norm(v: np.ndarray) -> np.ndarray:
        ...
        if not good.any():
            return out
        lo = float(np.nanmin(arr[good]))
        hi = float(np.nanmax(arr[good]))
        if hi - lo < 1e-12:
            out[good] = 1.0
        else:
    ...
    strong_n = _norm(strongest)
    mean_n = _norm(mean_rss)
    span_n = _norm(rss_span)

    rng = np.random.default_rng(int(seed))
    jitter = rng.uniform(0.0, 1.0, len(pool))
    ...
    q = float(np.nanquantile(base_quality, 0.85))
    first_candidates = np.flatnonzero(base_quality >= q - 1e-12)
    first = int(first_candidates[np.argmax(base_quality[first_candidates] + 0.02 * jitter[first_candidates])])
    ranking: list[int] = [first]

    # Only the requested prefix is needed.  The previous implementation ranked
    # *every* receiver location even when k was only 10--15, which made the
    # nested selector dominate runtime on large measurement tables.  Because
    # the ranking is deterministic, stopping at k yields exactly the same first
    # k locations as constructing the full ranking and slicing it afterwards.
    target_k = int(k)
    chosen_mask = np.zeros(len(pool), dtype=bool)
    chosen_mask[first] = True
    while len(ranking) < target_k:
        chosen = np.asarray(ranking, dtype=int)
        remaining = np.flatnonzero(~chosen_mask)
        if len(remaining) == 0:
            break

        dist = np.sqrt(
            ((xy[remaining, None, :] - xy[chosen][None, :, :]) ** 2).sum(axis=2)
        ).min(axis=1)
        dist_n = _norm(dist)

        selected_rss = rss[chosen]
        diversity = np.zeros(len(remaining), dtype=float)
        for ii, idx in enumerate(remaining):
            diffs = []
            for ref in selected_rss:
                common_mask = np.isfinite(rss[idx]) & np.isfinite(ref)
                if common_mask.any():
                    diffs.append(float(np.mean(np.abs(rss[idx, common_mask] - ref[common_mask]))))
            diversity[ii] = min(diffs) if diffs else 0.0
        diversity_n = _norm(diversity)

        score = (
            0.48 * dist_n
            + 0.22 * coverage_n[remaining]
            + 0.14 * diversity_n
            + 0.10 * strong_n[remaining]
            + 0.06 * span_n[remaining]
            + 0.010 * jitter[remaining]
        )
        next_idx = int(remaining[int(np.nanargmax(score))])
        ranking.append(next_idx)
        chosen_mask[next_idx] = True

    idx = np.asarray(ranking[:target_k], dtype=int)
    ...
    out["selection_strategy"] = "nested_information_balanced_prefix"
    return out
...
def build_collocated_simulation_dataset(
    ...
    *,
    ...
    simulation_floor_weight: float = 0.20,
    ...
    for sector_index in range(1, sector_count + 1):
        ...
        if sector_rows.empty:
            ...
            simulation_selected[f"obs_weight_s{sector_index}"] = 0.0
        ...
        try:
            search_root = simulation_root if simulation_root.exists() else project_root
            cache_key = (str(Path(search_root).expanduser().resolve()), int(station_id), int(pci))
            path = _SIMULATION_PATH_CACHE.get(cache_key)
            if path is None:
                path = discover_simulation_prior(
                    search_root, station_id=int(station_id), pci=pci,
                )
                _SIMULATION_PATH_CACHE[cache_key] = Path(path).resolve()
            prior = _SIMULATION_PRIOR_CACHE.get(cache_key)
            if prior is None:
                prior = load_simulation_prior(path, station_id=int(station_id), pci=pci)
                _SIMULATION_PRIOR_CACHE[cache_key] = prior
        ...
        if fill_missing_with_floor:
            floor_weight = float(np.clip(simulation_floor_weight, 0.0, 1.0))
            observation_weight = np.where(direct_valid, 1.0, floor_weight)
        else:
            observation_weight = np.where(direct_valid, 1.0, 0.0)
        simulation_selected[f"obs_weight_s{sector_index}"] = observation_weight
    ...
    diagnostics = {
        ...
        "simulation_floor_observation_weight": float(np.clip(simulation_floor_weight, 0.0, 1.0)),
    ...
    for i in range(len(selected)):
        for j in range(sector_count):
            ...
            if omni:
                ...
                if direction_used:
                    feat.append(gain)
            weight_col = f"obs_weight_s{j + 1}"
            obs_weight = float(selected.iloc[i][weight_col]) if weight_col in selected.columns else 1.0
            if not np.isfinite(obs_weight) or obs_weight <= 0.0:
                continue
            rows_X.append(feat)
            rows_y.append(float(y))
            rows_w.append(float(np.clip(obs_weight, 1e-3, 1.0)))
            location_ids.append(i)
            sector_ids.append(j)
    return (
        np.asarray(rows_X, float),
        np.asarray(rows_y, float),
        np.asarray(rows_w, float),
        np.asarray(location_ids, int),
        np.asarray(sector_ids, int),
    )
...
def _ridge_fit(
    ...
    if sample_weight is None:
        ...
        w = np.clip(np.where(np.isfinite(w), w, 0.0), 1e-6, 1.0)

    sw = np.sqrt(w)
    Xw = X * sw[:, None]
    yw = y * sw
    A = Xw.T @ Xw + np.diag(penalty)
...
def _lolo_cv_score(tx: np.ndarray, selected: pd.DataFrame, angles: np.ndarray, direction_used: bool, omni: bool) -> dict:
    """Weighted leave-one-location-out prediction score for one candidate position.

    Measurement observations have unit weight.  Simulation values that came from
    actual RT cells also have unit weight, whereas lower-censored no-path fills
    carry a smaller weight.  This keeps sparse simulation coverage from dominating
    the localization objective merely because it was completed at -120 dBm.
    """
    X, y, obs_w, loc_ids, _ = _validation_rows(tx, selected, angles, direction_used, omni)
    if len(y) < max(5, X.shape[1] + 1):
        return {"cv_rmse_db": 1e6, "cv_mae_db": 1e6, "cv_bias_db": np.nan, "fit_rmse_db": np.nan,
                ...
                "effective_observation_weight": float(np.sum(obs_w))}
    ...
    for loc in unique_locs:
        ...
        if int(train.sum()) < X.shape[1] + 1 or int(test.sum()) == 0:
            continue
        beta = _ridge_fit(X[train], y[train], omni, direction_used, obs_w[train])
        pred = X[test] @ beta
        residuals.extend((pred - y[test]).tolist())
        residual_weights.extend(obs_w[test].tolist())
    if not residuals:
        return {"cv_rmse_db": 1e6, "cv_mae_db": 1e6, "cv_bias_db": np.nan, "fit_rmse_db": np.nan,
                "pathloss_exponent": np.nan, "gain_scale": np.nan, "parameter_boundary_penalty": 10.0,
                "effective_observation_weight": float(np.sum(obs_w))}
    residuals_arr = np.asarray(residuals, float)
    residual_weights_arr = np.asarray(residual_weights, float)
    residual_weights_arr = np.clip(residual_weights_arr, 1e-6, 1.0)
    beta = _ridge_fit(X, y, omni, direction_used, obs_w)
    fit_res = X @ beta - y
    ...
    wsum = float(np.sum(residual_weights_arr))
    fit_wsum = float(np.sum(obs_w))
    return {
        "cv_rmse_db": float(np.sqrt(np.sum(residual_weights_arr * residuals_arr ** 2) / max(wsum, 1e-12))),
        "cv_mae_db": float(np.sum(residual_weights_arr * np.abs(residuals_arr)) / max(wsum, 1e-12)),
        "cv_bias_db": float(np.sum(residual_weights_arr * residuals_arr) / max(wsum, 1e-12)),
        "fit_rmse_db": float(np.sqrt(np.sum(obs_w * fit_res ** 2) / max(fit_wsum, 1e-12))),
        "pathloss_exponent": float(n),
        "gain_scale": gain_scale,
        "parameter_boundary_penalty": float(penalty),
        "effective_observation_weight": fit_wsum,
    }
...
def score_candidate(
    ...
    if simulation_selected is None or float(simulation_weight) <= 0.0:
        return {
            **measured_cv,
            ...
            "simulation_reliability": float("nan"),
            "effective_simulation_weight": 0.0,
        }
    ...
    base_weight = float(np.clip(simulation_weight, 0.0, 1.0))
    sim_weight_values = []
    for j in range(1, 4):
        c = f"obs_weight_s{j}"
        r = f"rsrp_s{j}"
        if c in simulation_selected.columns and r in simulation_selected.columns:
            mask = pd.to_numeric(simulation_selected[r], errors="coerce").notna().to_numpy()
            vals = pd.to_numeric(simulation_selected[c], errors="coerce").to_numpy(float)
            sim_weight_values.extend(vals[mask & np.isfinite(vals)].tolist())
    reliability = float(np.mean(sim_weight_values)) if sim_weight_values else 1.0
    reliability = float(np.clip(reliability, 0.05, 1.0))
    # Reliability-adjusted odds: direct RT samples keep the requested weight;
    # floor-completed/no-path samples reduce the effective simulation influence.
    denom = (1.0 - base_weight) + base_weight * reliability
    weight = float((base_weight * reliability) / max(denom, 1e-12))
    ...
    return {
        **measured_cv,
        ...
        "effective_simulation_weight": weight,
...
def solve(
    ...
    # Candidate points are intentionally revisited during coarse scoring, local
    # refinement, final deduplication and progressive line search.  Cache their
    # LOLO scores within this solve so each unique coordinate is evaluated once.
    evaluation_cache: dict[tuple[float, float], dict] = {}
    ...
    def evaluate(point: np.ndarray) -> dict:
        ...
        value = score_candidate(
            p, selected, angles, direction_used, omni,
    ...
    if previous_xy is not None and np.isfinite(np.asarray(previous_xy, float)).all():
        ...
        # Progressive trust-region continuation.  Instead of allowing an added
        # receiver location to make an arbitrary jump, search only along the
        # segment from the inherited solution toward the new unconstrained
        # solution.  alpha=0 is always included, so the active validation
        # objective can never become worse when the point count increases.
        delta = np.asarray(unconstrained_xy, float) - prev
        distance = float(np.linalg.norm(delta))
        max_step = float(np.clip(0.35 * common.point_spread(selected) + 20.0, 45.0, 120.0))
        if distance > max_step and distance > 1e-12:
            target = prev + delta * (max_step / distance)
        else:
            target = np.asarray(unconstrained_xy, float)

        alphas = np.asarray([0.0, 0.10, 0.20, 0.35, 0.50, 0.70, 0.85, 1.0], dtype=float)
        path_scored = []
        for alpha in alphas:
            point = _clip_candidate(prev + float(alpha) * (target - prev), bounds)
            ev = evaluate(point)
            path_scored.append((float(alpha), point, ev))
        path_scored.sort(key=lambda item: item[2]["candidate_score"])
        alpha_best, path_xy, path_best = path_scored[0]

        improvement = previous_score - float(path_best["candidate_score"])
        # In the M+S branch use a weak Pareto guard: the joint score must improve,
        # while neither measured nor simulation validation is allowed to degrade
        # materially.  This prevents one channel from dragging the position away.
        pareto_ok = True
        if simulation_selected is not None:
            measured_ok = float(path_best.get("measured_cv_rmse_db", np.inf)) <= float(prev_eval.get("measured_cv_rmse_db", np.inf)) + 0.20
            simulation_ok = float(path_best.get("simulation_cv_rmse_db", np.inf)) <= float(prev_eval.get("simulation_cv_rmse_db", np.inf)) + 0.20
            pareto_ok = bool(measured_ok and simulation_ok)

        if improvement < float(progressive_min_improvement_db) or not pareto_ok or alpha_best <= 0.0:
            best_label, best_xy, best = "previous_count_solution_retained", prev, prev_eval
            progressive_update_accepted = False
        else:
            best_label = f"progressive_line_search_alpha_{alpha_best:.2f}"
            best_xy, best = np.asarray(path_xy, float), path_best
            progressive_update_accepted = True
    ...
    return {
        ...
        "effective_simulation_weight": float(best.get("effective_simulation_weight", 0.0)),
        "simulation_reliability": float(best.get("simulation_reliability", np.nan)),
