...
def discover_simulation_prior(
    ...
    # ``project_root`` may be either the repository root or an explicitly
    # supplied simulation-map root (used by localization/tests).  Repository
    # auto-discovery is deliberately restricted to the formal DEM+1.5 m map
    # directory so stale products from other receiver-surface experiments are
    # never selected.
    if (project_root / "run_pipeline.py").is_file() or (project_root / "outputs").is_dir():
        roots = [project_root / "outputs" / "bestparam_radio_maps_512m"]
    else:
        roots = [project_root]
