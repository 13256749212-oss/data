...
def build_all_27_station_configs(
    ...
    for station_id, group in raw_long.groupby("station_id", sort=True):
        ...
        # 22号站按用户现场确认强制作为单PCI全向站处理。
        # 即使后续输入表中的antenna_type标记出现偏差，也不再进入三扇区alpha校验。
        omni = bool(
            sid == 22
            or group["is_omnidirectional"].fillna(0).astype(int).max()
        )
        ...
        if omni:
            pci_counts = group["pci"].dropna().astype(int).value_counts()
            if pci_counts.empty:
                raise ValueError(f"{sid}号全向站没有有效PCI")
            # 当前正式数据中22号站为单PCI。若未来文件意外包含多个PCI，
            # 使用记录数最多的PCI作为全向载波，避免错误进入三扇区逻辑。
            selected_pci = int(pci_counts.index[0])
            pcis = (selected_pci,)
            if len(pci_counts) > 1:
                print(
                    f"警告：{sid}号全向站检测到多个PCI {sorted(pci_counts.index.tolist())}，"
                    f"本次选用记录数最多的PCI {selected_pci}。"
                )
...
def main() -> None:
    ...
    cfg["project"]["measurement_csv"] = "data/cell_pci_rsrp_long_27stations.csv"
    cfg["search"]["power_min_dbm"] = 52.0
    cfg["search"]["power_max_dbm"] = 55.0
    cfg["search"]["power_step_db"] = 0.3
    ...
    power_grid = np.arange(52.0, 55.0 + 0.15, 0.3)
    power_grid[-1] = 55.0
